...
> **当前版本**：`2.0.1.20260603-SNAPSHOT`<br>
> **JDK 基线**：`17`<br>
...
**xxljob-spring-boot-starter** 是一个面向 使用 xxljob 的应用 的 Spring Boot Starter，用于将 **xxljob** 集成到 Spring Boot 应用中。它提供自动装配、属性绑定与开箱即用的 Bean，使应用以最小配置即可使用 xxljob 的全部能力。

| 维度 | 说明 |
|---|---|
| 类型 | Spring Boot Starter |
| 消费方 | 使用 xxljob 的 Spring Boot 应用 |
| 核心能力 | 自动装配、属性绑定、开箱即用的 xxljob Bean |
| JDK | `17` |
| 坐标 | `io.github.easy4j:xxljob-spring-boot-starter:2.0.1.20260603-SNAPSHOT` |
| 配置前缀 | `xxljob` |
...
| 能力 | 状态 | 说明 |
|---|:---:|---|
| 自动装配 | ✅ 稳定 | 自动注册 xxljob 相关 Bean |
| 属性绑定 | ✅ 稳定 | 绑定 `xxljob.*` 到 `XxlJobAdminCookieProperties` |
| `XxlJobTemplate` Bean | ✅ 稳定 | 通过 XxlJobAutoConfiguration, XxlJobMetricsAutoConfiguration 自动注册 |
